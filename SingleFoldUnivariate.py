...
def single_fold(k, percentage, dataset, kernel, cmin,cmax,number_of_cs,rank_metric=f_classif):
        ...
        output_directory = os.path.join(get_full_path(outer_directory),'{}-f_classif'.format(dataset))
        try:
...
# single_fold(k=1, percentage=5, dataset='sick', kernel='linear', cmin=0, cmax=7, number_of_cs=8,rank_metric=chi2)