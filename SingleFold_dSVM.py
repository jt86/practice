...
def single_fold(k, top_k, dataset, datasetnum, kernel, cmin, cmax, number_of_cs, skfseed, percent_of_priv, percentageofinstances, take_top_t):
        ...
        output_directory = get_full_path(('Desktop/Privileged_Data/dSVM295-FIXEDC-10x10-{}-ALLCV{}to{}-featsscaled-step{}-{}{}percentpriv-{}percentinstances/tech{}/top{}chosen-{}percentinstances/').format(dataset, cmin, cmax, stepsize, percent_of_priv, take_top_t, percentageofinstances, datasetnum, top_k, percentageofinstances))
        ...
        with open(os.path.join(cross_validation_folder, 'lupi-{}-{}-C={}.csv'.format(k, top_k, c)), 'a') as cv_lupi_file:
            cv_lupi_file.write(str(accuracy_lupi) + ',')

        print('svm+ accuracy=\n',accuracy_lupi,'C={}'.format(c))
