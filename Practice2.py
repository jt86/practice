...
def get_l1_ordered_indices(all_training,training_labels):
    lsvc = LinearSVC(C=0.01, penalty="l1", dual=False).fit(all_training, training_labels)
    print(all_training.shape)
    model = SelectFromModel(lsvc, prefit=True, threshold='median')
    X_new = model.transform(all_training)
    print(len(np.where(model.get_support()==True)[0]))
    print(X_new.shape)
    # sys.exit()
...
def single_fold(k, topk, dataset, datasetnum, kernel, cmin, cmax, number_of_cs, skfseed, percent_of_priv, percentageofinstances, take_top_t, featsel):
    ...
    output_directory = get_full_path(('Desktop/Privileged_Data/PRACTICESubset-{}-10x10-{}-ALLCV{}to{}-featsscaled-step{}-{}percentinstances/{}{}/top{}chosen-{}percentinstances/').format(featsel, dataset, cmin, cmax, stepsize, percentageofinstances, dataset, datasetnum, topk, percentageofinstances))
    ...
    # sys.exit()
    if featsel =='l1':
        ordered_indices = get_l1_ordered_indices(all_training, training_labels)
...
for datasetnum in range(1):
    single_fold(k=1, topk=300, dataset='tech', datasetnum=datasetnum, kernel='linear', cmin=-3, cmax=3, number_of_cs=7,
                            skfseed=1, percent_of_priv=100, percentageofinstances=100, take_top_t='bottom', featsel='l1')
#
# for skfseed in range(7,10):
#     for k in range(10):
