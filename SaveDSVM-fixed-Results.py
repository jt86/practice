...
count=0
dsvm_lufe=[]
for dataset_num in range(34):
    all_folds_baseline, all_folds_SVM, all_folds_lufe1 = [], [], []
    for seed_num in range (num_repeats):
        output_directory = ('/Volumes/LocalDataHD/j/jt/jt306/Desktop/dSVM295-FIXEDCpoint1-10x10-tech-ALLCV-3to3-featsscaled-step0.1-100toppercentpriv-100percentinstances-1/tech{}/top{}chosen-100percentinstances/cross-validation{}'.format(dataset_num, n_top_feats,seed_num))
...
list_of_dsvm_errorsFIXED = np.array([1 - mean for mean in np.mean(dsvm_lufe, axis=1)]) * 100
dsvm_error_bars_295 = list(stats.sem(dsvm_lufe, axis=1) * 100)

print(list_of_dsvm_errorsFIXED)
...
improvements_list = compare_two_settings(list_of_dsvm_errors,list_of_dsvm_errorsFIXED,'cross-val','fixed')
