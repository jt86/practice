...
class SVMdp():
    ...
    def train(self, prob : svm_problem):
        ...
        #h4 = np.zeros(self.L).reshape(-1,1)
        h = np.vstack((h2, h3))
        #h = np.vstack((h, h4))
        ...
        P = matrix(H, tc='d')
        #print("p is size "+str(P.size))
        #print("rank P = "+str(mr(P)))
        q = matrix(f, tc='d')
        #print("rank q = "+str(mr(q)))
        G = matrix(G, tc='d')
        #print("rank G = "+str(mr(G)))
        h = matrix(h, tc='d')
        #print("rank h = "+str(mr(h)))
        A = matrix(Aeq, tc='d')
        #print("rank A = "+str(mr(A)))
        b = matrix(beq, tc='d')
        #print("rank b = "+str(mr(b)))
